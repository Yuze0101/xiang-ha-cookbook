<template>
	<view class="">
		...
		<!-- nav部分 -->
		<view class="nav">
			<view class="nav-item" v-for="(item, index) in nav" :key="item._id" @click="navigateTo(index, item.name)">
		...
		<!-- 美食展示部分 -->
		<view class="food">
			<view class="food-item" v-for="item in foods" :key="item._id" @tap="toDetail(item._id)">
				...
				<view class="p2">
					...
					<text class="p-right">{{ item.collections }} </text>
		...
		methods: {
			...
			navigateTo(index, name) {
				...
				if (index === this.nav.length - 1) {
					wx.navigateTo({
						url: "/pages/index/categories",
					})
				} else {
					uni.navigateTo({
						url:"/pages/index/search?search="+name
					})
				}
			},
			//跳转详情页
